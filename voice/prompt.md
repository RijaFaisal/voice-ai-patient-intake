...
registration calls. It checks for an existing patient via the
`lookup_patient` function (`GET /patients/lookup`), saves collected data
via the `save_patient` function (`POST /patients`), and can book a first
appointment via the `schedule_appointment` function (`POST /appointments`).
